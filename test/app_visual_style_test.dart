...
void main() {
  testWidgets('Mac glass renders frosted surfaces without UIKit platform views', (tester) async {
    if (!Platform.isMacOS || !AppVisualStyle.supportsLiquidGlass) return;
    await tester.pumpWidget(PlatformProvider(
      style: AppVisualStyle.liquidGlass,
      builder: (_) => MaterialApp(home: Builder(builder: (context) {
        expect(usesAppleTranslucentSurface(context), isTrue);
        expect(usesLiquidGlass(context), isFalse);
        return const PlatformLiquidGlassCard(child: Text('Mac glass'));
      })),
    ));
    expect(find.byType(PlatformGlassBackdrop), findsOneWidget);
    expect(find.byType(UiKitView), findsNothing);
    expect(tester.takeException(), isNull);
  });

  test(
    'Mac system appearance chooses glass on 26+, without overriding Material',
    () {
      expect(
        AppVisualStyle.system.resolve(
          isIOS: false,
          isMacOS: true,
      ...
      expect(
        AppVisualStyle.material.resolve(
          isIOS: false,
          isMacOS: true,
          supportsLiquidGlass: true,
        ),
      ...
      expect(
        AppVisualStyle.macOSSupportsGlass('Version 27.0 (Build 26A428)'),
        isTrue,
      );
      expect(
        AppVisualStyle.macOSSupportsGlass('Version 15.6 (Build 24G84)'),
        isFalse,
      );
      expect(AppVisualStyle.macOSSupportsGlass('unknown'), isFalse);
    },
  );
  ...
  test(
    'legacy iOS selection keeps adaptive glass, not classic-only Cupertino',
    () {
  ...
  for (final brightness in Brightness.values) {
    testWidgets(
      'Cupertino profile keeps its colored card background ($brightness)',
      (tester) async {
        ...
        expect(
          tester.widget<Text>(find.text('Credits')).style!.color,
          Colors.white,
        );
        expect(
  ...
  for (final brightness in Brightness.values) {
    testWidgets(
      'Cupertino incognito menu keeps foreground and background paired ($brightness)',
      (tester) async {
        ...
        expect(
          tester.widget<Text>(find.text('匿名模式')).style!.color,
          colors.onPrimary,
        );
        expect(
          tester.widget<Text>(find.text('某些功能可能不可用')).style!.color,
          colors.onPrimary,
        );
        expect(tester.takeException(), isNull);
      },
  ...
  for (final locale in [const Locale('en'), const Locale('zh', 'CN')]) {
    testWidgets(
      ...
      (tester) async {
        ...
        if (!AppVisualStyle.supportsLiquidGlass) {
