...
class UserPreferencesUtils {
  ...
  static final String themeColorNameKey = "themeColorNameKeyV2";
  static final String customThemeColorKey = "customThemeColorKeyV1";

  static Future<void> putThemeColor(FlexScheme themeColor) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setString(themeColorNameKey, themeColor.name);
    await prefs.remove(customThemeColorKey);
  }

  static Future<void> putCustomThemeColor(Color color) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setInt(customThemeColorKey, color.toARGB32());
  }

  static Future<Color?> getCustomThemeColor() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    final value = prefs.getInt(customThemeColorKey);
  ...
  static Future<FlexScheme> getThemeColor() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    final storedName = prefs.getString(themeColorNameKey);
    if (storedName != null) {
      for (final scheme in getAvailableThemes()) {
        if (scheme.name == storedName) return scheme;
      }
    }

    // Migrate the legacy array-index value defensively. Enum values can grow
    // between package versions, so an out-of-range preference must not prevent
    // the application from starting.
    final availableThemes = getAvailableThemes();
    final colorIndex = prefs.getInt(themeColorKey);
    if (colorIndex != null &&
        colorIndex >= 0 &&
        colorIndex < availableThemes.length) {
      final migrated = availableThemes[colorIndex];
      await prefs.setString(themeColorNameKey, migrated.name);
      return migrated;
    }
  ...
  static const String appleIntelligenceGuardrailKey =
      "appleIntelligenceGuardrailKeyV1";

  static Future<void> putAppleIntelligenceEnabled(bool enabled) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(appleIntelligenceEnabledKey, enabled);
  }

  static Future<bool> getAppleIntelligenceEnabled() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getBool(appleIntelligenceEnabledKey) ?? false;
  }

  static Future<void> putAppleIntelligenceGuardrail(String level) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(appleIntelligenceGuardrailKey, level);
  }

  static Future<String> getAppleIntelligenceGuardrail() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getString(appleIntelligenceGuardrailKey) ?? "standard";
  }
  ...
  static final String typesettingScalePreferenceKey =
      "typesettingScalePreferenceKey";
  ...
  static final String interfaceBrightnessPreferenceKey =
  ...
  static final String disableFontCustomizationKey =
      "disableFontCustomizatioKey";
  ...
  static Future<String> getSignaturePreference() async {
    ...
    return signaturePreference == null
  ...
  static final String hapticFeedbackPreferenceKey =
      "hapticFeedbackPreferenceKey";
  ...
  static final String subscribedChannelPreferenceKey =
  ...
  static final String typographyThemePreferenceKey =
      "typographyThemePreferenceKey";
  ...
  static final String useCompactParagraphPreferenceKey =
      "useCompactParagraphPreferenceKey";
  ...
  static final String adExemptDiscuzHostPreferenceKey =
      "adExemptDiscuzHostPreferenceKey";
