...
enum AppVisualStyle {
  ...
  static final bool supportsLiquidGlass =
      PlatformInfo.isIOS26OrHigher() ||
      (Platform.isMacOS && macOSSupportsGlass(Platform.operatingSystemVersion));

  static bool macOSSupportsGlass(String version) {
    final match = RegExp(r'(?:Version\s+)?(\d+)\.').firstMatch(version);
    return match != null && int.parse(match.group(1)!) >= 26;
  }
  ...
  AppVisualStyle resolve({
    ...
    final canUseGlass = isApple && supportsLiquidGlass;
    return switch (this) {
      system => isApple ? (canUseGlass ? liquidGlass : cupertino) : material,
