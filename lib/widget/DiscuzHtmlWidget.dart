import 'NeteaseMusicWidget.dart';
...
class DiscuzHtmlWidget extends StatelessWidget {
  ...
  /// Use only as a direct sliver in the page's existing CustomScrollView.
  final bool asSliver;

  /// Called when parsed body content is ready for scroll-position restoration.
  ...
  Widget build(BuildContext context) {
    final content = Consumer<TypeSettingNotifierProvider>(
      builder: (context, typesetting, _) {
        ...
        String platformName = Provider.of<ThemeNotifierProvider>(
          context,
          listen: false,
        ).platformName;
        ...
        TextTheme textTheme = typography.tall.useSystemChineseFont(
        ...
        final repairedSourceHtml = ReadingPerformanceProbe.measure(
          'html.normalizeTable',
          () => DiscuzTableNormalizer.normalizeHtml(html),
        );
        final normalizedHtml = ReadingPerformanceProbe.measure(
          'html.decodeAndWrapLinks',
          () => PostTextUtils.getDecodedString(
            repairedSourceHtml,
            useCompactParagraph,
          ),
        );
        if (normalizedHtml.trim().isEmpty && onBodyReady != null) {
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (context.mounted) onBodyReady!();
          });
        }
        //DiscuzImageDioCacheManager dioCacheManager = DiscuzImageDioCacheManager(futureDio);

        final rendered = HtmlWidget(
          normalizedHtml,
          renderMode: asSliver ? RenderMode.sliverList : RenderMode.column,
          factoryBuilder: () => _ReadingHtmlWidgetFactory(onBodyReady),
          //enableCaching: true,
          onTapUrl: (url) {
          ...
          customWidgetBuilder: (element) {
            if (element.localName == 'a' && onContentChanged != null) {
              final target = ForumPurchaseTarget.parse(
                discuz.baseURL,
                element.attributes['href'],
                tid,
              );
              if (target != null)
                return ForumActionButton(
                  discuz: discuz,
                  tid: target.tid,
                  aid: target.aid,
                  purchase: true,
                  label: target.aid == null
                      ? S.of(context).forumBuyThread
                      : S.of(context).forumBuyAttachment,
            ...
            if (element.localName == 'pre' ||
              ...
              if (element.attributes["time"] != null) {
                ...
                element.attributes["href"]!.startsWith(
                  "https://www.bilibili.com",
                )) {
              return BilibiliWidget(element.attributes["href"]!);
            }
            if (element.localName == 'iframe') {
              final music = NeteaseMusicEmbed.parse(element.attributes['src']);
              if (music != null)
                return NeteaseMusicWidget(
                  music,
                  key: ValueKey('netease-${music.songId}'),
                );
            }
        ...
        return asSliver
            ...
            : ReadingPerformanceProbe.layout('html.layout', rendered);
      },
    );
    return asSliver
        ? SliverPadding(padding: const EdgeInsets.all(4), sliver: content)
        : Container(
...
/// Keep only visited blocks alive: expansion state, media and horizontal table
/// offsets survive scrolling away. Unvisited blocks are still laid out lazily.
class _ReadingHtmlWidgetFactory extends WidgetFactory {
  final VoidCallback? onBodyReady;
  _ReadingHtmlWidgetFactory(this.onBodyReady);
  ...
  Widget buildBodyWidget(BuildContext context, Widget child) {
    ...
    if (body is! SliverList || body.delegate is! SliverChildBuilderDelegate) {
      return body;
    }
    final delegate = body.delegate as SliverChildBuilderDelegate;
    // Keep the canonical RenderMode.sliverList: version 0.17.3 uses identity
    // equality to choose sliver placeholders during asynchronous HTML parsing.
    return SliverList(
  ...
  Widget buildAnchorBodyItem(BuildContext context, int index, Widget widget) =>
      _RetainedReadingBlock(
        child: super.buildAnchorBodyItem(context, index, widget),
      );
}

class _RetainedReadingBlock extends StatefulWidget {
  final Widget child;
  const _RetainedReadingBlock({required this.child});
  @override
  State<_RetainedReadingBlock> createState() => _RetainedReadingBlockState();
}

class _RetainedReadingBlockState extends State<_RetainedReadingBlock>
