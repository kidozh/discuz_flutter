...
class ManageDiscuzState extends State<ManageDiscuzStateWidget> {
  ...
  Widget _buildSecureIcon(BuildContext context, Discuz discuz) {
    if (discuz.baseURL.startsWith("http://")) {
      return Icon(AppPlatformIcons(context).inSecureHttpWarningSolid,
          color: Theme.of(context).splashColor);
    }
    if (discuz.isDiscuzVersion35) {
      return Icon(AppPlatformIcons(context).supportDiscuz35Solid,
          color: Theme.of(context).colorScheme.primary);
    } else {
      return Icon(
        AppPlatformIcons(context).secureHttpsSolid,
                    ...
                    onTap: () {
                      ...
                      Navigator.push(
                          ...
                          platformPageRoute(
                              ...
                              builder: (context) =>
                                  ExclusiveDiscuzPortalPage(discuz)));
                    },
                  ),
  ...
  _deleteDiscuz(Discuz discuz) async {
    if (_discuzDao != null) {
      ...
      EasyLoading.showSuccess(
          S.of(context).deleteDiscuzSuccessfully(discuz.siteName));
    }
  }
}
