...
import 'support/reading_scenario.dart';
import 'support/local_vm_timeline.dart';
...
Future<void> main() => runThermalBenchmark();

Future<void> runThermalBenchmark(
    {bool reading = false, bool diagnose = false}) async {
  final binding = WidgetsFlutterBinding.ensureInitialized();
  final tester = LiveWidgetController(binding);
  FlutterErrorDetails? frameError;
  final previousErrorHandler = FlutterError.onError;
  FlutterError.onError = (details) {
    frameError = details;
    previousErrorHandler?.call(details);
  };
  void checkFrameErrors() {
    if (frameError != null) throw StateError(frameError!.exceptionAsString());
  }

  _require(kProfileMode, 'A physical iPhone profile build is required.');
  ...
  final timeline = LocalVmTimeline();
  var phase = 'setup';
  var lastLogSecond = -10;
  final report = <String, dynamic>{
    'completed': false,
    'scenario': reading ? 'heavy_thread_reading' : 'headline_scrolling',
    'diagnostic_instrumentation': diagnose,
    'mode':
        'profile, physical iPhone, standard WidgetsFlutterBinding; no forced frame loop',
    'scope': 'Production KeylolMobileTopicWidget / ForumThreadWidget / '
        'ViewThreadSliverPage / PostWidget / DiscuzHtmlWidget when reading; '
  ...
  final watchdog = Timer(const Duration(minutes: 8), () {
  ...
  try {
    ...
    _require(device['appIdentifier'] == 'com.kidozh.discuz-flutter.perftest',
        'Wrong app bundle');
    _require(device['isSimulator'] == false, 'Physical device required');
    if (diagnose) {
      ReadingPerformanceProbe.enabled = true;
      await timeline.start();
      report['timeline_unavailable'] = timeline.unavailable;
    }
    final refreshRate = (device['maximumFramesPerSecond'] as num).toDouble();
    _require(refreshRate > 0, 'The native display must be ready.');
    ...
    runApp(MultiProvider(
      providers: [
        ...
        ChangeNotifierProvider(create: (_) => DiscuzNotificationProvider()),
        ChangeNotifierProvider(create: (_) => ReplyPostNotifierProvider()),
        ChangeNotifierProvider(create: (_) => ThemeNotifierProvider()),
        ChangeNotifierProvider(create: (_) => TypeSettingNotifierProvider()),
        ChangeNotifierProvider(create: (_) => UserPreferenceNotifierProvider()),
      ],
      child: PlatformProvider(
        ...
        builder: (_) => MaterialApp(
          ...
          builder: (context, child) => Column(children: [
            Expanded(child: ToastUtils.easyLoadingBuilder()(context, child)),
            Material(
                child: SafeArea(
                    top: false,
                    child: Padding(
          ...
          home: PlatformScaffold(
            ...
            body: reading
                ? buildReadingSampleList(source.discuz!)
                : const KeylolMobileTopicWidget(),
          ),
        ),
      ),
    ));
    if (reading) {
      ...
      report['reading'] = await runReadingScenario(tester,
          onStarted: (partial) => report['reading'] = partial,
          diagnose: diagnose,
          shouldStop: () => stopReason != null,
          sample: sample,
          gestures: gestures,
          measure: (name, label, action) async {
            ...
            phases.add(stage);
            if (diagnose) ReadingPerformanceProbe.drain();
            try {
              await sample();
              await action();
              await sample();
              ...
              stage['actual_duration_s'] =
                  stageWatch.elapsedMilliseconds / 1000;
              if (diagnose) {
                stage['probe_events'] = ReadingPerformanceProbe.drain();
                stage['timeline'] = await timeline.take();
              }
            }
          });
    } else {
      ...
      while (tabs.evaluate().isEmpty) {
        ...
        if (loading.elapsed > const Duration(seconds: 90)) {
          throw StateError(
              'Live Keylol topics did not load within 90 seconds.');
        }
      }
      _require(
          usesLiquidGlass(tester.element(tabs)), 'Liquid Glass must be active');
      ...
      ScrollPosition position() => tester
          ...
          .position;
      void verify() {
        _require(list.evaluate().length == 1, 'Exactly one active list');
        _require(
            find
                    .descendant(
                        of: categories, matching: find.byType(UiKitView))
                    .evaluate()
                    .length ==
                1,
            'Native category control required');
        _require(
            find
                .descendant(of: list, matching: find.byType(BackdropFilter))
                .evaluate()
                .isEmpty,
            'No per-row backdrop blur');
        _require(
            find
                .descendant(
                ...
                .evaluate()
                .isNotEmpty,
            'Production glass cards required');
        checkFrameErrors();
      }
        ...
        try {
          ...
          while (stageWatch.elapsed < duration && stopReason == null) {
            if (scroll) {
              ...
              _require(extent > 50, 'List must be scrollable');
              ...
              _require(
                  (after - before).abs() > 20, 'Gesture must move the list');
              gestures.add({
                'elapsed_s': watch.elapsedMilliseconds / 1000,
    ...
    await Future<void>.delayed(
        const Duration(seconds: 1)); // Flush batched timings.
    ...
    report['completed'] = stopReason == null &&
        (reading ? report['reading']['completed'] == true : phases.length == 3);
    ...
    if (diagnose) await timeline.close();
